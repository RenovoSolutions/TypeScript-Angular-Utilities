...
export interface IMockPromiseService {
	...
	flushAll(service: any): Promise<void>;
}
...
class MockPromiseService implements IMockPromiseService {
	...
	flushAll(service: any): Promise<void> {
		return <Promise<any>> Promise.all(
			_.map(service, (promise: IMockedPromise<any>): Promise<void> => {
	...
	private makeDynamicMockPromise<TData>(result: { (...args: any[]): TData }, shareParam: boolean): IMockedPromiseInternal<TData> {
		...
		mocked.flush = (): Promise<void> => {
			...
			var flushOperation: Promise<void> = <Promise<any>> Promise.all(
				requests.map(request => request.promise)
			).catch(() => null);
			// swallow errors intentionally generated by mock.rejectedPromise
			requests = [];
